...
namespace API.Controllers
{
    ...
    [ApiController]
    public class EnrollmentsCertificatesController : ControllerBase
    {

        TSTDBContext _context;
          ILogger<GeneralCertificatesController> _logger;
        private readonly FileUploadService _fileUploadService;
        public EnrollmentsCertificatesController(TSTDBContext context, ILogger<GeneralCertificatesController> logger, FileUploadService fileUploadService)
        {
        ...
        public async Task<IActionResult> Get(string studentNumber)
        {
            try
            {
                var result = await _context.EnrollmentRequests
                    .Include(s => s.Service)
                    .Include(s => s.Student)
                    .Include(s => s.Status)
        ...
        // GET api/<EnrollmentsCertificatesController>/5
        [HttpGet("{id}")]
        ...
        public async Task<IActionResult> Post([FromBody] CreateCertificateRequestDTO dto)
        {
            try
            {
                ...
                var service = await _context.Services.FirstOrDefaultAsync(s => s.Name == "Enrollment Certificate");
                if (student != null)
                {
                    var certificateRequest = new EnrollmentRequest
                    {
                        Student = student,
                        Status = status,
                        Service = service,
                    ...
                    await _context.EnrollmentRequests.AddAsync(certificateRequest);
                    ...
                    if (result > 0)
                    {
                        var Payment = new Payment
                        {
                            ...
                            EnrollmentRequestId = certificateRequest.Id,
                            EnrollmentRequest = certificateRequest,
                            CreatedAt = DateTime.Now,
                            Amount = service.Fee,
        ...
        public async Task<IActionResult> Put(string id, [FromBody] UpdateCertificateRequestDTO dto)
        {
            try
            {
                ...
                var certificateRequest = _context.EnrollmentRequests.Include(c => c.Status).FirstOrDefault(cr => cr.Id == id);
                if (certificateRequest != null)
                {
                    ...
                    _context.EnrollmentRequests.Update(certificateRequest);
